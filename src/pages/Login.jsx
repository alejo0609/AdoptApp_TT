...
import useSessionTimeout from "../hooks/useSession"; // ✅ tu nuevo hook
...
function Login() {
  ...
  const { iniciarSesion, cerrarSesion } = useSession();

  // ✅ Hook para cerrar sesión si hay inactividad
  ...
  // Este hook estará "activo" después del login exitoso
  useSessionTimeout(handleInactividad); // ⏱️
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.mensaje === "Inicio de sesión exitoso") {
        ...
        iniciarSesion({ email }); // guarda sesión
        navigate("/adopta"); // redirige
      } else {
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-sm text-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Link
              to="/registro"
              className="inline-block bg-yellow-500 text-white px-4 py-2 rounded-md font-bold hover:bg-yellow-600 mt-2"
            >
              Registrarse
            </Link>
          </div>
